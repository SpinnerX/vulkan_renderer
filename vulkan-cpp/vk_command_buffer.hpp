...
namespace vk {
    ...
    struct command_buffer_properties {
        command_buffer_properties(uint32_t p_queue_family, const command_buffer_levels& p_levels, const VkCommandPoolCreateFlagBits& p_pool_flags) : QueueIndex(p_queue_family), Levels(p_levels), PoolFlags(p_pool_flags) {}
    ...
    */
    ...
    public:
        ...
        // checks if command buffer is valid because we also check if begin and end are setup correctly
        bool is_valid_operation() const { return (m_begin_end_count == 2); }

        // checks if VkCommandBuffer is valid
        bool is_handle_valid() const { return (m_command_buffer_handler != nullptr); }
    ...
    private:
        VkDevice m_driver;
        // This is just to indicate to throw an error if you call end before begin or begin and no end for this command buffer
        // To at least give us an indication if there is no begin/end being called for this specific command buffer
        // If begin/end are both called respectively, this counter should be 2.
