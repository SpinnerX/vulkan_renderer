...
namespace vk {
    vk_index_buffer::vk_index_buffer(const std::span<uint32_t>& p_indices) {
        ...
        VkDeviceSize buffer_size = p_indices.size_bytes(); // this should do exactly as what I am thinking it does
    ...
    void vk_index_buffer::bind(const VkCommandBuffer& p_command_buffer) {
        vkCmdBindIndexBuffer(p_command_buffer, m_index_buffer_data.BufferHandler, 0, VK_INDEX_TYPE_UINT32);
    }
