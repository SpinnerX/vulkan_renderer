...
namespace vk {
    ...
    buffer_properties create_uniform_buffer(uint32_t p_size) {
        ...
        buffer_properties buffer_data = create_buffer(p_size, flags, memory_property_flag);

        return buffer_data;
    }

    vk_uniform_buffer::vk_uniform_buffer(const uint32_t& p_image_size, uint32_t p_size_in_bytes) {
    ...
    void vk_uniform_buffer::destroy() {
        for(size_t i = 0; i < m_uniform_buffers.size(); i++) {
            vkFreeMemory(m_driver, m_uniform_buffers[i].DeviceMemory, nullptr);
            vkDestroyBuffer(m_driver, m_uniform_buffers[i].BufferHandler, nullptr);
        }
    }
};